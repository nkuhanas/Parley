...
test("OpenClaw adapter client mode materializes remote plan projection mirrors", async () => {
  await withTempRoot(async (tempRoot) => {
    const calls = [];
    const mirrorRoot = path.join(tempRoot, "projection-mirror");
    const serviceLocalPath = path.join(tempRoot, "service", "repo", "plans", "drafts", "remote-plan.md");
    const body = "# Remote Plan\n\nGenerated by service.\n";
    const tools = registeredTools({
      env: openclawEnv(tempRoot),
      pluginConfig: {
        parleyMode: "client",
      ...
      fetchImpl: async (url, init) => {
        ...
        return jsonResponse({
          ...
          data: {
            ...
            plan: { plan_id: "plan_remote", uri: "repo://plans/drafts/remote-plan.md" },
            projection: {
              kind: "plan_markdown",
              uri: "repo://plans/drafts/remote-plan.md",
